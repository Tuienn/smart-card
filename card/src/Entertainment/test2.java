...
//class này ch ð test và ðýc ðy lên ð tránh khi m project b li	
public class test2 extends Applet {

    /* constants declaration */

    // code of CLA byte in the command APDU header
    final static byte Entertainment_CLA = (byte) 0x80;
...
//    byte[] encryptedSecret = new byte[64];
    ...
    private test2(byte[] bArray, short bOffset, byte bLength) {
    ...
    private void encryptPin (APDU apdu) {
    	 ...
    	 try {
    	     ...
    	     rsaCipher.init(smartcard_rsa_public, Cipher.MODE_ENCRYPT);
    	     short encLen = rsaCipher.doFinal(secret, (short) 0, (short) secret.length, outBuffer, (short) 0);
    	     apdu.sendBytes((short) 0, encLen);
    ...
    private byte[] decryptPin (byte[] encryptedPin) {
   	 try {
   	     ...
   	     rsaCipher.init(smartcard_rsa_private, Cipher.MODE_DECRYPT);
   	     short decLen = rsaCipher.doFinal(encryptedPin, (short) 0, (short) encryptedPin.length, outBuffer, (short) 0);
   	     byte[] result = new byte[decLen];
   	     for(short i = 0; i < decLen; i++) {
    ...
    public static void install(byte[] bArray, short bOffset, byte bLength) {
        // create a Entertainment applet instance
        new test2(bArray, bOffset, bLength);
    ...
    public void process(APDU apdu) {
        ...
        // command structure
        if (buffer[ISO7816.OFFSET_CLA] != Entertainment_CLA) {
            ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
        }
    ...
    private void get_cardholder_methods(APDU apdu) {
        ...
        short len = (short)(xValue.length + yValue.length + cvmRules.length);
        apdu.setOutgoingLength(len);
        
        byte[] buffer = apdu.getBuffer();
        short bufferLength = 0;
        for(short xIndex=0; xIndex < xValue.length; xIndex++) {
        	buffer[bufferLength] = xValue[xIndex];
        	bufferLength++;
        }
        
        for(short yIndex=0; yIndex < yValue.length; yIndex++) {
        	buffer[bufferLength] = yValue[yIndex];
        	bufferLength++;
        }
        
        for(short cvmIndex=0; cvmIndex < cvmRules.length; cvmIndex++) {
        	buffer[bufferLength] = cvmRules[cvmIndex];
        	bufferLength++;
        }
        
        apdu.sendBytes((short) 0, bufferLength);
    }
} // end of class Entertainment
